                    ...
                    try:
                        ...
                        if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                            try:
                                ...
                                message = f'[B][C]{get_random_color()}\n- Welcome to Emote Bot ! \n\n{get_random_color()}- Commands: /dance <team> <uid> <emote>\n\n[00FF00]Emote Service Ready'
                                P = await SEndMsG(0 , message , OwNer_UiD , OwNer_UiD , key , iv)
                                await SEndPacKeT(whisper_writer , online_writer , 'ChaT' , P)
                            except:
                                pass
    ...
    while True:
        try:
            ...
            if LoGinDaTaUncRypTinG.Clan_ID:
                ...
                print(f' - Connected to Clan Chat Successfully!')
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        if inPuTMsG in ("hi" , "hello" , "fen" , "salam"):
                            uid = response.Data.uid
                            chat_id = response.Data.Chat_ID
                            message = 'Hello! Emote Bot is Online\nCommands: /dance <team> <uid> <emote>'
                            P = await SEndMsG(response.Data.chat_type , message , uid , chat_id , key , iv)
                            await SEndPacKeT(whisper_writer , online_writer , 'ChaT' , P)
                        response = None
...
async def perform_emote(team_code: str, uids: list, emote_id: int):
    """🔄 تنفيذ إيموت سريع مع خروج فوري"""
    global key, iv, region, online_writer
    
    if online_writer is None:
        raise Exception("⚠️ Bot is not connected")
    
    try:
        # 🚀 STEP 1: Join team FAST (0.2s delay)
        join_packet = await GenJoinSquadsPacket(team_code, key, iv)
        await SEndPacKeT(None, online_writer, 'OnLine', join_packet)
        await asyncio.sleep(0.2)  # تأخير قصير جداً
        
        # 🎭 STEP 2: Execute emotes FAST
        emote_tasks = []
        for uid_str in uids:
            if uid_str and uid_str.strip():  
                uid = int(uid_str.strip())
                emote_packet = await Emote_k(uid, emote_id, key, iv, region)
                emote_tasks.append(emote_packet)
        
        # إرسال جميع الإيموتات بسرعة
        for packet in emote_tasks:
            await SEndPacKeT(None, online_writer, 'OnLine', packet)
            await asyncio.sleep(0.05)  # تأخير بسيط جداً
        
        # 🚪 STEP 3: Exit IMMEDIATELY (0.1s delay only)
        await asyncio.sleep(0.1)  # تأخير قصير جداً قبل الخروج
        leave_packet = await ExiT(None, key, iv)
        await SEndPacKeT(None, online_writer, 'OnLine', leave_packet)
        
        # 📊 الإحصائيات
        total_time = 0.2 + (len(emote_tasks) * 0.05) + 0.1
        
        return {
            "status": "success", 
            "message": "⚡ Emote executed and exited instantly",
            "details": {
                "team": team_code,
                "targets": len(emote_tasks),
                "emote": emote_id,
        ...
        # Emergency exit in case of error
        try:
            leave_packet = await ExiT(None, key, iv)
            await SEndPacKeT(None, online_writer, 'OnLine', leave_packet)
        except:
        ...
        raise Exception(f"❌ Quick emote failed: {str(e)}")

@app.route('/join')
def join_team():
    """API endpoint for fast emote execution"""
    global loop
    ...
    if not team_code or not emote_id_str:
        return jsonify({
            "status": "error", 
            "message": "❌ Missing required parameters: tc (team code) and emote_id"
        })
    ...
    try:
        ...
        return jsonify({
            "status": "error", 
            "message": "❌ emote_id must be a valid number"
        })

    # Collect UIDs
    uids = [uid for uid in [uid1, uid2, uid3, uid4] if uid and uid.strip()]
    ...
    try:
        ...
        return jsonify({
            "status": "success",
            "data": result,
            "note": "⚡ Task completed in under 1 second"
        })
        
    except asyncio.TimeoutError:
        return jsonify({
            "status": "error", 
            "message": "⌛ Timeout - Bot might be busy or disconnected"
        })
    except Exception as e:
        return jsonify({
            "status": "error", 
...
def bot_status():
    ...
    return jsonify({
        ...
        "message": "✅ Bot is online and ready" if online_writer else "❌ Bot is offline"
    })
...
async def MaiiiinE():
    ...
    os.system('clear')
    print(render('drox-1', colors=['cyan', 'blue'], align='center'))
    print('')
